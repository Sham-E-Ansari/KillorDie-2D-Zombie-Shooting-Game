...
void iShowBMP(int x, int y, char filename[])
{
	...
	//glDrawPixels(TextureImage->sizeX, (TextureImage->sizeY)-64, GL_RGB, GL_UNSIGNED_BYTE, TextureImage->data);
...
void iShowBMPBack(int x, int y, char filename[])
{
	AUX_RGBImageRec *TextureImage;
...
//MODIFIED BY SAIFUR RAHMAN
void iShowBMPWOBG(int x, int y, char filename[])
{
	AUX_RGBImageRec *TextureImage;
	TextureImage = auxDIBImageLoad(filename);
...
/*
	...
	Send the color you want to make transperant as a parameter (After the file)
	The colors Has to of int values , not hex.

	To Determine The color in int go to : http://www.shodor.org/stella2java/rgbint.html
	To detimine the color in RGB use Photoshop.
*/
void iShowBMPWOBGWP(int x, int y, char filename[],int col)
{
	AUX_RGBImageRec *TextureImage;
	TextureImage = auxDIBImageLoad(filename);
